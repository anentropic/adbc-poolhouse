...
The async API is experimental and incomplete. Its surface may change between minor releases, and several features (DataFrame fetches, async metadata, and prepared statements) are not available yet. See the [async pool guide](guides/async.md) for the full caveat.
