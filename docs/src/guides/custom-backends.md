...
`connect()` must fall into one of three shapes:
...
| Has `db_kwargs`; `uri` required-positional (no default) | `connect(uri, db_kwargs={...})` with `uri` popped from kwargs first | PostgreSQL, FlightSQL, Quack |
