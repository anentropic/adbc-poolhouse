...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/adbc_poolhouse/_driver_api.py"
      provides: "create_adbc_connection() ADBC facade"
...
<objective>
Wire the Phase 4 infrastructure: (1) `_translators.py` dispatch coordinator, (2) `_drivers.py` 3-path driver detection, (3) `_driver_api.py` ADBC facade with all type suppressions, (4) minimal `_pool_types.py` type alias scaffold.
...
def create_adbc_connection(
    ...
    *,
    ...
    are concentrated in this module. ADBC exceptions pass through raw.
...
"</automated>
    <manual>Check _drivers.py has no top-level warehouse driver imports (grep for adbc_driver_snowflake, adbc_driver_duckdb, etc. at module level). Check _driver_api.py has all type: ignore comments concentrated in it.</manual>
  </verify>
  <done>
...
- create_adbc_connection() is the single location for all ADBC type suppressions
