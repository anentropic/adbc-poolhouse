...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 26-01-xx | 01 | 1 | PKG-01 | — | N/A | unit/build | `uv pip show adbc-poolhouse` + parse `[async]` extra; `uv lock --locked` | ❌ W0 | ⬜ pending |
| 26-02-xx | 02 | 1 | PKG-05 | — | N/A | type | `.venv/bin/basedpyright src/adbc_poolhouse/_async` → 0 errors; negative-arg probe flags mistyped arg | ✅ | ⬜ pending |
| 26-03-xx | 03 | 1 | PKG-02, PKG-03 | — | guard raises ImportError naming `[async]` when anyio absent | unit (subprocess) | `.venv/bin/pytest tests/test_async_import_guard.py` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/test_async_import_guard.py` — subprocess-isolated regression tests for PKG-02/03 (import succeeds anyio-absent; async-symbol access raises ImportError naming `[async]`)
