...
<objective>
Run a final integration verification that all Wave 1 and Wave 2 plans integrate correctly, then pause for the human to register OIDC trusted publishers on PyPI and TestPyPI (no CLI exists for this — web UI only).
...
<task type="auto">
  ...
  <action>
Run four integration checks to confirm all prior plans integrated correctly:
